...
MIN_DEC_STEPS=100
MAX_DEC_STEPS=120
CUDA_VISIBLE_DEVICES=0 python run_summarization.py --mode=decode --data_path="$DATA_PATH" --vocab_path="$VOCAB_PATH" --log_root="$LOG_ROOT" --exp_name="$EXP_NAME" --single_pass --coverage --max_enc_steps="$MAX_ENC_STEPS" --min_dec_steps="$MIN_DEC_STEPS" --max_dec_steps="$MAX_DEC_STEPS" &  O=$!

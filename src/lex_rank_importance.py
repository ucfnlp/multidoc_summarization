...
class LexRankSummarizer(AbstractSummarizer):
    ...
    def __call__(self, document, sentences_count, tokenizer):
        ratings = self.get_ratings(document, tokenizer)
        return self._get_best_sentences(document.sentences, sentences_count, ratings)

    # Added by Logan to just get ratings
    def get_ratings(self, document, tokenizer):
        scores = self.get_importances(document, tokenizer)
        ratings = dict(zip(document.sentences, scores))
        return ratings

    # Added by Logan to just get sentence importances
    def get_importances(self, sentences, tokenizer):
        self._ensure_dependencies_installed()

        sentences_words = [self._to_words_set(s, tokenizer) for s in sentences]
